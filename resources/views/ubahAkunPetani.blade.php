...
<body class="font-urbanist bg-black min-h-screen">
    ...
    <main class="p-6 bg-white rounded-b-xl">
        ...
        <form action="{{ route('updatePetani',['id' => $petani->ID_Akun] )}}" method="POST" class="max-w-5xl mx-auto mt-6">
            @csrf
            {{-- @method('PUT') --}}
            <!-- Avatar -->
            ...
            <div class="bg-white shadow-md rounded-lg p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="Nama" value="{{ old('Nama', $petani->Nama) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">

                        <label class="block font-semibold text-gray-700 mt-4">Alamat</label>
                        <input type="text" name="Alamat" value="{{ old('Alamat', $petani->Alamat) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">

                        <label class="block font-semibold text-gray-700 mt-4">No.Hp</label>
                        <input type="text" name="Nomor_HP" value="{{ old('Nomor_HP', $petani->Nomor_HP) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">
                    </div>

                    <!-- Kanan -->
                    <div>
                        <label class="block font-semibold text-gray-700">Username</label>
                        <input type="text" name="Username" value="{{ old('Username', $petani->Username) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">

                        <label class="block font-semibold text-gray-700 mt-4">Email</label>
                        <input type="email" name="Email" value="{{ old('Email', $petani->Email) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">

                        <label class="block font-semibold text-gray-700 mt-4">Sandi</label>
                        <input type="text" name="Sandi" value="{{ old('Sandi', $petani->Sandi) }}" class="w-full border border-pink-600 rounded px-3 py-2 mt-1">
                    </div>
                </div>
